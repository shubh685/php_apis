...
// =====================================================
// ACTION 1: SEND OTP (5-DIGIT, 2 MINUTES EXPIRY)
// =====================================================
if ($action === 'send_otp') {
    ...
    try {
        ...
        // Generate 5-Digit OTP and store hash in database
        $otp = (string) random_int(10000, 99999);
        $otpHash = password_hash($otp, PASSWORD_DEFAULT);
        // Expiry set strictly to 2 minutes
        ...
        try {
            ...
            $mail->setFrom("shahshubham128@gmail.com", "Grow Logix");
            $mail->addAddress($user['email'], $user['name'] ?? 'User');

            $mail->isHTML(true);
            $mail->Subject = "Grow Logix - Password Reset OTP";

            $mail->Body = "
                <div style='font-family:Arial,sans-serif;max-width:600px;margin:auto;'>
                    <h2 style='color:#3D1F03;'>Grow Logix</h2>
                    ...
                    <div style='background:#f5f1e8;padding:25px;text-align:center;margin:20px 0;'>
                        <div style='font-size:13px;color:#666;'>Your 5-Digit OTP</div>
            ...
            $mail->AltBody = "Your OTP is: {$otp}. Valid for 2 minutes.";
            $mail->send();

            echo json_encode([
                "status" => true,
                "message" => "5-digit OTP sent successfully to your registered email (valid for 2 mins)"
            ]);
            exit();
...
// =====================================================
// ACTION 2: VERIFY OTP (5-DIGIT)
// =====================================================
if ($action === 'verify_otp') {
    $input = cleanInput($data['input'] ?? $data['email'] ?? $data['username'] ?? $data['emp_id'] ?? $data['mobile'] ?? '');
    $otp   = trim((string)($data['otp'] ?? ''));

    if (empty($input) || !preg_match('/^[0-9]{5}$/', $otp)) {
        http_response_code(422);
        echo json_encode([
            "status" => false,
            "message" => "Valid identifier and a 5-digit OTP are required"
        ]);
        exit();
    }
